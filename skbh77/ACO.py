...
def ACO():
    ...
    while t<max_it: #sets
        ...
        pToAdd = [[ 0 for _ in range(num_cities) ] for _ in range(num_cities)]
        for k in range(0,N):
            currPos = antTours[k][-1]
            for i in range(1,num_cities):
                newVert = calculateEdge(antTours[k],pLevels,currPos)
                costs[k] += dist_matrix[currPos][newVert]
                antTours[k].append(newVert)      
                currPos = newVert
            costs[k] += dist_matrix[currPos][antTours[k][0]]
            if costs[k] < minTour[1]:
                minTour = antTours[k],costs[k]
            for j in range(0,len(antTours[k])-1):
                pToAdd[antTours[k][j]][antTours[k][j+1]] += 1/costs[k]
                pToAdd[antTours[k][j+1]][antTours[k][j]] += 1/costs[k]
