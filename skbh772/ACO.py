...
def sumEdgeWeights(forbidden,pLevels,currentVert):
    total = 0
    for i in range(0,len(dist_matrix)):
        if i not in forbidden:
            hDesirability = float(1/(dist_matrix[currentVert][i]))
            currP = float(pLevels[currentVert][i])
            p = ( (currP)**a * (hDesirability**b))
            total += p
    return total

def calculateEdge(forbidden,pLevels,currentVert):
    bestP = -1
    bestEdge = -1
...
def updatePheromones(toAdd,currP):
    for i in range(0,len(currP)):
        for j in range(0,len(currP[i])):
            currP[i][j] = ((1-p) * currP[i][j]) + toAdd[i][j]
    return currP
...
def ACO():
    ...
    #startingPos = random.choices(range(0, num_cities), N)
    #print(statingPos)
    t = 0
    max_it = 1
    startTime = time.time();
    ##startCosts = [ 0 for _ in range(N) ]
    basePToAdd = [[ 0 for _ in range(num_cities) ] for _ in range(num_cities)]
    while True: 
        ##costs = startCosts[:]
        pToAdd = basePToAdd[:]
        minTour = [bTour,NN]
        for k in range(0,N):
            ...
            if time.time()-startTime > 7200:
                note = "Ants: Num of cities // 2. Iterations: : " +str(t)
                return bestTour[0],bestTour[1],note;
            for i in range(1,num_cities):
                newVert = calculateEdge(antTour,pLevels,currPos)
            ...
            for j in range(0,len(antTour)-1):
                pToAdd[antTour[j]][antTour[j+1]] += 1/cost
                pToAdd[antTour[j+1]][antTour[j]] += 1/cost
        if minTour[1] < bestTour[1]:
            bestTour = minTour[:]
        pLevels = updatePheromones(pToAdd,pLevels)
        t += 1
    note = "Ants: 100. Iterations: : " +str(t)
    return bestTour[0],bestTour[1],note
