...
def addToList(arr,tour,cost,):
    ...
    while(left<=right):
        mid = (right+left)//2
        if cost >= arr[mid-1][0] and cost<=arr[mid][0]:
            temp = arr[mid:len(arr)-1]
            arr[mid] = [cost,tour]
            arr[mid+1:] = temp
            return arr
        else:
...
def ACO():
    ...
    print(NN)
    P0 = float((0.5*w*(w-1))/(NN))
    #P0 = float(N/NN)
    bestWTours = [[9999999999999999,0] for x in range(w)] 
    ...
    while True:
        ...
        for k in range(0,N):
            ...
            if cost<bestWTours[-1][0]:
                bestWTours = addToList(bestWTours,antTour,cost)
            if cost < minTour[1]:
        ...
        for i in range(0,len(bestTour)-1):
            bestnum1 = bestTour[0][i]
            bestnum2 = bestTour[0][i+1]
            pUpdated[bestnum1][bestnum2] += (w)*(1/bestTour[1])
            pUpdated[bestnum2][bestnum1] += (w)*(1/bestTour[1])
