...
##def createMatrix(N,tours,tour):
##    neighbourMatrix = [0 for col in range(N)] 
##    for i in range(0,len(tours)):
##        if i!=tour:
##            neighbourMatrix[i] = getDistance(tours[tour],tours[i])
##    return neighbourMatrix

    
def getNeighbour(N,tours,tour,bestCosts):
    #matrix = createMatrix(N,tours,tour)
    ...
    while(True):
        for a in range(0,N):
            if time.time()-startTime > 300:
                print(t)
                return pBest
            #neighbour = getNeighbour(N,bestTours,a,bestCosts)
