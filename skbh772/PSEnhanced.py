...
def opt():
    ...
    #routeCost = calculateDistance(route)
    bestRoute = route
...
def getNeighbour(N,tours,tour,bestCosts):
    smallestNeigbourCost = 9999999999
    smallestNeighbour = 0
    for i in range(0,N):
        if i!=tour:
            dist = getDistance(tours[tour],tours[i])
            if(dist <= eps and bestCosts[i]<smallestNeigbourCost):
                smallestNeighbour = tour
                smallestNeigbourCost = bestCosts[i]
    return tours[smallestNeighbour]
